...
1. **`SUMMARY.md`** — artifact list, transcript provenance, backend, and read order.
2. **`metadata.md`** — source-platform post text/title, creator, date, engagement, and
   chapters when the extractor exposes them.
3. **`timeline.md`** — the most useful view: each frame next to what was said and what
   was on screen at that timestamp.
4. **`transcript.md`** — the full timestamped transcript.
5. **`frames/*.jpg`** — open individual frames. For an exact on-screen number, read
...
### Caption-first and CUDA

The default order is authored caption, automatic caption, then local Whisper. For local
